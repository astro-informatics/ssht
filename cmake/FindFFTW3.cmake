# Distributed under the MIT License Taken from github.com/UCL/GreatCMakeCookOff
#
# * Try to find FFTW
#
# By default, it will look only for the serial libraries with single, double, and long
# double precision. Any combination of precision (SINGLE, DOUBLE, LONGDOUBLE) and
# library type (SERIAL, [THREADS|OPENMP], MPI) is possible by using the COMPONENTS
# keyword. For example,
#
...
# * FFTW3_$KIND_$PARALLEL_FOUND: set if FFTW3 exists in KIND precision format for
#   PARALLEL mode. where KIND can be: SINGLE, DOUBLE, LONGDOUBLE and PARALLEL: SERIAL,
#   OPENMP, MPI, THREADS.
...
macro(find_specific_libraries KIND PARALLEL)
    ...
    find_library(
        ...
        NAMES fftw3${SUFFIX_${KIND}}${SUFFIX_${PARALLEL}}${SUFFIX_FINAL}
              libfftw3${SUFFIX_${KIND}}${SUFFIX_${PARALLEL}}${SUFFIX_FINAL}
        HINTS ${HINT_DIRS})
    if(FFTW3_${KIND}_${PARALLEL}_LIBRARY MATCHES fftw3)
        ...
        # adding target properties to the different cases MPI
        if(PARALLEL STREQUAL "MPI")
            if(MPI_C_LIBRARIES)
                set_target_properties(
                    fftw3::${kind}::mpi
...
# FIXME - This may fail in computers wihtout serial Default serial to obtain version
# number
...
if(WIN32)
    ...
    set(HINT_DIRS ${PC_FFTW3_INCLUDE_DIR} ${PC_FFTW3_INCLUDE_DIRS} ${FFTW3_INCLUDE_DIR}
...
function(find_version OUTVAR LIBRARY SUFFIX)
    file(
        WRITE ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/fftw${SUFFIX}/main.c
...
if(WIN32)
    ...
    set(HINT_DIRS ${PC_FFTW3_LIBDIR} ${PC_FFTW3_LIBRARY_DIRS} $ENV{FFTW3_LIBRARY_DIR}
                  ${FFTW3_LIBRARY_DIR})
endif(WIN32)
...
# handle the QUIETLY and REQUIRED arguments and set FFTW3_FOUND to TRUE if all listed
# variables are TRUE
