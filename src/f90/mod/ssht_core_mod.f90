...
module ssht_core_mod
  ...
  ! DH sampling does not have South pole sample so no alternative interface.
  ...
  ! GL sampling does not have South pole sample so no alternative interface.
...
!!$                  + Fmm(m,-mm) * exp(-I*mm*theta)
                  Fmm(m,mm) * &
                  (exp(I*mm*theta) + signs(abs(m)) * ssign * exp(-I*mm*theta))
          end do
       end do
    end do
...
!    call dfftw_destroy_plan(fftw_plan)
  ...
  subroutine ssht_core_mw_forward_sov_conv_sym(flm, f, L, spin, verbosity)
    ...
    ! Extend Fmt periodically.
    do m = -(L-1), L-1
       Fmt(m, L:2*L-2) = signs(abs(m)) * ssign * Fmt(m, L-2:0:-1)
    end do
