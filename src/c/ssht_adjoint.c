...
//	Fmm[(m+Fmm_offset)*Fmm_stride + mm + Fmm_offset];
  ...
  for (m=-(L-1); m<=L-1; m++) {
    ...
    for(mm=0; mm<=L-1; mm++)
      inout[mm] = Gmm[(mm+Fmm_offset)*Fmm_stride + m + Fmm_offset];
    for(mm=-(L-1); mm<=-1; mm++)
      inout[mm+2*L-1] = Gmm[(mm+Fmm_offset)*Fmm_stride + m + Fmm_offset];
    fftw_execute_dft(plan, inout, inout);
...
//============================================================================
  ...
  /* Gmm_full_offset = L-1; */
  /* Gmm_full_stride = 2*L-1; */
  /* Gmm_full = (complex double*)calloc((2*L-1)*(2*L-1), sizeof(complex double)); */

  /* for (m=0; m<=L-1; m++)  */
  /*   for(mm=-(L-1); mm<=L-1; mm++) */
  /*     Gmm_full[(mm+Gmm_full_offset)*Gmm_full_stride + m + Gmm_full_offset] = */
  /* 	Gmm[(mm+Fmm_offset)*Fmm_stride + m]; */

  /* for (m=-(L-1); m<=-1; m++)  */
  /*   for(mm=-(L-1); mm<=L-1; mm++) */
  /*     Gmm_full[(mm+Gmm_full_offset)*Gmm_full_stride + m + Gmm_full_offset] = */
  /* 	conj(Gmm[(mm+Fmm_offset)*Fmm_stride - m]); */
  ...
  /* free(Gmm); */
  /* Gmm = (complex double*)calloc((2*L-1)*(2*L-1), sizeof(complex double)); */
  /* memcpy(Gmm, Gmm_full, (2*L-1)*(2*L-1)*sizeof(complex double)); */
  /* free(Gmm_full); */
  ...
  /*   for(p=0; p<=2*L-2; p++) */
  /*     f[t*f_stride + p] = creal(inout[p]) / (2.0*L-1.0); */
...
//	Fmm[(m+Fmm_offset)*Fmm_stride + mm + Fmm_offset];
  ...
  for (m=-(L-1); m<=L; m++) {
    ...
    for(mm=0; mm<=L; mm++)
      inout[mm] = Gmm[(mm+Gmm_offset)*Gmm_stride + m + Gmm_offset];
    for(mm=-(L-1); mm<=-1; mm++)
      inout[mm+2*L-1+1] = Gmm[(mm+Gmm_offset)*Gmm_stride + m + Gmm_offset];
    fftw_execute_dft(plan, inout, inout);
