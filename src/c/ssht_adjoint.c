...
void ssht_adjoint_mw_forward_sov_sym_ss_real(double *f, 
 ...
 // Compute Fmm.
  ...
  tmp_pad = (complex double*)calloc(4*L-3, sizeof(complex double));
  SSHT_ERROR_MEM_ALLOC_CHECK(tmp_pad)
  Gmm = (complex double*)calloc((2*L)*(L+1), sizeof(complex double));
  Gmm_stride = L+1;
  Gmm_offset = L-1;
//  Gmm = (complex double*)calloc((2*L-1)*L, sizeof(complex double));
  //Gmm_stride = L;
...
//	Fmm[m*Fmm_stride + mm + Fmm_offset];
    ...
    // Extract section of Gmm of interest.
    for (mm=-(L-1); mm<=L-1; mm++)
      Gmm[(mm+Gmm_offset)*Gmm_stride + m] = 
	Fmm_pad[mm + w_offset] * 2.0 * SSHT_PI / (4.0*L-3.0);
...
//============================================================================
  ...
  for (m=0; m<=L; m++) {
    ...
    for(mm=0; mm<=L; mm++)
      inout[mm] = Gmm[(mm+Gmm_offset)*Gmm_stride + m];
    for(mm=-(L-1); mm<=-1; mm++)
      inout[mm+2*L-1+1] = Gmm[(mm+Gmm_offset)*Gmm_stride + m];
    fftw_execute_dft(plan, inout, inout);
